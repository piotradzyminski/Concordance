...
(function initEquipmentCyberwareLink() {
  ...
  function getCyberwareWorkspaceRoot(citizenId = "") {
    ...
    if (!root) return null;
    const id = String(citizenId || "").trim();
    return !id || String(root.dataset.equipmentCitizenId || "").trim() === id ? root : null;
  }
  ...
  function invalidateCyberwareWorkspaceRuntime(citizenId = "", options = {}) {
    ...
    const screen = root?.querySelector?.('[data-equipment-screen="CYBERWARE"]') || null;
    if (screen) screen.dataset.equipmentScreenDirty = "true";
    const plannerHost = screen?.querySelector?.("[data-cyberware-planner-host]") || null;
    if (plannerHost && options.planner !== false) plannerHost.dataset.cyberwarePlannerDirty = "true";
    const diagnosticsHost = screen?.querySelector?.("[data-cyberware-diagnostics-host]") || null;
    if (diagnosticsHost && options.diagnostics !== false) diagnosticsHost.dataset.cyberwareDiagnosticsDirty = "true";
    const maintenanceHost = screen?.querySelector?.("[data-cyberware-maintenance-host]") || null;
  ...
  function openEquipmentWorkspace(citizenId = "", user = window.WS_APP.currentUser, options = {}) {
    const normalizedCitizenId = String(citizenId || "").trim();
    if (normalizedCitizenId) window.WS_APP.equipmentTargetCitizenId = normalizedCitizenId;
    ...
    return `
      <section class="equipment-shell-panel" data-equipment-panel="cyberware-link">
        <div class="equipment-shell-panel__head">
          <p class="kicker">EQUIPMENT / CYBERWARE LINK</p>
          <h5>Cyberware Workspace</h5>
        </div>
        <p class="equipment-shell-copy">Cyberware remains a separate workspace. Equipment exposes the canonical ItemInstance link point for installation and service operations.</p>
        <button class="secondary-action" type="button" data-equipment-cyberware-link ${citizenId ? "" : "disabled"}>Open Cyberware Context</button>
      </section>
    `;
  }
  ...
  function renderEquipmentCyberwareWorkspace(state = {}, citizen = {}, options = {}) {
    const citizenId = String(citizen?.id || state?.citizenId || "").trim();
    ...
    return `<section class="equipment-cyberware-workspace cyberware-ui-workspace" data-equipment-panel="cyberware-workspace" data-cyberware-active-section="${activeSection}" data-cyberware-active-view="${activeView}">
  ...
  function refreshEquipmentCyberwareWorkspace(citizenId = "", options = {}) {
    const id = String(citizenId || "").trim();
    const root = getCyberwareWorkspaceRoot(id);
    const screen = root?.querySelector?.('[data-equipment-screen="CYBERWARE"]') || null;
    const workspace = screen?.querySelector?.('[data-equipment-panel="cyberware-workspace"]') || null;
    const citizen = getCitizen(id);
    if (!screen || !workspace || !citizen) return false;
  ...
  app.openEquipmentWorkspace = openEquipmentWorkspace;
  app.renderEquipmentCyberwareLinkPanel = renderEquipmentCyberwareLinkPanel;
  app.renderEquipmentCyberwareWorkspace = renderEquipmentCyberwareWorkspace;
  ...
  app.refreshEquipmentCyberwareWorkspace = refreshEquipmentCyberwareWorkspace;
