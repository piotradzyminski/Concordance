...
(function initCitizenStoreModule() {
  ...
  window.WS_APP.addCitizenSubscription = function addCitizenSubscription(citizenId, subscription = {}) {
    const citizen = window.WS_APP.getCitizenById(citizenId);
    if (!citizen || citizen.recordType === "admin") return null;
  ...
  window.WS_APP.updateCitizenSubscription = function updateCitizenSubscription(citizenId, subscriptionId, patch = {}, options = {}) {
    const citizen = window.WS_APP.getCitizenById(citizenId);
  ...
  window.WS_APP.cancelCitizenSubscription = function cancelCitizenSubscription(citizenId, subscriptionId, options = {}) {
    const citizen = window.WS_APP.getCitizenById(citizenId);
  ...
  window.WS_APP.deleteCitizenSubscription = function deleteCitizenSubscription(citizenId, subscriptionId, options = {}) {
    const citizen = window.WS_APP.getCitizenById(citizenId);
    if (!citizen || !subscriptionId) return null;
  ...
  window.WS_APP.clearCancelledCitizenSubscriptions = function clearCancelledCitizenSubscriptions(citizenId, options = {}) {
    const citizen = window.WS_APP.getCitizenById(citizenId);
    if (!citizen || citizen.recordType === "admin") return null;
  ...
  function normalizeTerminalEntry(entry = {}) {
    ...
    const type = normalizeTerminalEntryType(entry?.type);
    ...
    const actions = (Array.isArray(entry?.actions) ? entry.actions : [])
      .map((action) => {
        ...
        return {
          ...clone(action),
          actionId: normalizeNotificationToken(action.actionId, "OPEN"),
          label: String(action.label || action.actionId || "OPEN").trim().toUpperCase(),
          routeId: String(action.routeId || "").trim().toUpperCase(),
          entityRef: normalizeTerminalReference(action.entityRef || {}),
    ...
    return {
      ...
      domain,
      eventCode,
      category: normalizeNotificationToken(entry?.category || domain, domain),
      source,
      type,
      subtype,
      severity,
      ...
      links: (Array.isArray(entry?.links) ? entry.links : []).map(normalizeTerminalLink)
    };
  }
  ...
  window.WS_APP.processWeeklySubscriptionSettlement = function processWeeklySubscriptionSettlement(options = {}) {
    const settlementDateIso = String(options.settlementDateIso || window.WS_APP.getSettlementPeriodEndIso?.() || "").trim();
    ...
    citizenStore = citizenStore.map((citizen) => {
  ...
  window.WS_APP.payCitizenSubscriptions = function payCitizenSubscriptions(citizenId, options = {}) {
    const citizen = window.WS_APP.getCitizenById(citizenId);
    if (!citizen || citizen.recordType === "admin") return { ok: false, reason: "NO_CITIZEN" };
  ...
  const subscriptionStoreCommands = Object.freeze({
    addCitizenSubscription: window.WS_APP.addCitizenSubscription,
    updateCitizenSubscription: window.WS_APP.updateCitizenSubscription,
    cancelCitizenSubscription: window.WS_APP.cancelCitizenSubscription,
    removeCitizenSubscription: window.WS_APP.deleteCitizenSubscription,
    clearCancelledCitizenSubscriptions: window.WS_APP.clearCancelledCitizenSubscriptions,
    payCitizenSubscriptions: window.WS_APP.payCitizenSubscriptions,
    processWeeklySubscriptionSettlement: window.WS_APP.processWeeklySubscriptionSettlement,
